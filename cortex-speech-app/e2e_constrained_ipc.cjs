...
const { dataDir: DATA_DIR, ours: DATA_DIR_IS_OURS } = resolveDisposableProfile('e2e_constrained_ipc');
...
(async () => {
  await refuseIfDebugPortBusy(PORT, 'e2e_constrained_ipc');
  try {
  ...
  if (!hasArabic(text)) throw new Error(`constrained output has no Kurdish script at all: ${text}`);
  if (hasLatinLetters(text)) throw new Error(`constrained output leaked Latin text: ${text}`);
  console.log('\n=================================================');
  console.log(`  CONSTRAINED IPC OK: ${text} (${text.length} chars, Kurdish script, no Latin)`);
  console.log('=================================================');
