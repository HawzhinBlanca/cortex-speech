...
def policy() -> ReviewPilotPolicy:
    return ReviewPilotPolicy(863, 20, {"Sara": 10, "Hemn": 10})


def perfect_state() -> HiddenPilotState:
    grants = {
        "Sara": {"hidden-h-1", "hidden-h-2"},
        "Hemn": {"hidden-p-1", "hidden-p-2"},
    }
    return HiddenPilotState(
        ...
        skipped_keys={"Sara": set(), "Hemn": set()},
        unresolved_keys={"Sara": set(), "Hemn": set()},
        corpus_actions={"Sara": 10, "Hemn": 10},
        hidden_actions={"Sara": 2, "Hemn": 2},
    )
...
def perfect_events() -> list[gate.ReviewEventEvidence]:
    ...
    event_id = 864
    for reviewer, prefix in (("Sara", "h"), ("Hemn", "p")):
...
def test_zero_of_24_is_pending_not_a_capacity_green_certificate() -> None:
    empty = HiddenPilotState(
        policy_sha256=policy_sha256(policy()),
        grants={"Sara": set(), "Hemn": set()},
        session_keys={"Sara": set(), "Hemn": set()},
        completed_keys={"Sara": set(), "Hemn": set()},
        skipped_keys={"Sara": set(), "Hemn": set()},
        unresolved_keys={"Sara": set(), "Hemn": set()},
        corpus_actions={"Sara": 0, "Hemn": 0},
        hidden_actions={"Sara": 0, "Hemn": 0},
    )
...
def test_19_corpus_actions_cannot_clear_the_gate() -> None:
    ...
    assert any("Sara has 9/10 corpus" in item for item in found)
...
def test_hidden_quality_is_rederived_without_numeric_coercion_or_tolerance() -> None:
    for assignment, expected_error in (
        ...
        connection.execute(
            f"UPDATE spot_checks SET {assignment} WHERE segment_id = 'hidden-h-1' AND reviewer = 'Sara'"
        )
        quality, errors = gate.read_hidden_quality(connection, perfect_state())
        connection.close()
        assert ("Sara", "hidden-h-1") not in quality
...
def _playback_certificate_connection() -> sqlite3.Connection:
    ...
    connection.executescript(
        ...
        INSERT INTO review_compensation_ledger VALUES
            (864, 'review-iqd-v1-2026-08-21', 4, 1000, 'Sara', 'work-h-0', 'couch',
             'reviewer-work-v1:4:sara:audio-segment-v1:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa:0:1000',
...
def _manifest_connection() -> sqlite3.Connection:
    ...
    connection.executescript(
        ...
        INSERT INTO review_events VALUES (
            864, 'work-h-0', 'Sara', 'accept', 'couch',
        ...
        INSERT INTO review_compensation_ledger VALUES (
            ...
            864, 'Sara', 'work-h-0', 'couch', 4, NULL, 500000
        );
        INSERT INTO review_compensation_ledger VALUES (
            2, 'undo-entry-1', 'undo:22222222-2222-4222-8222-222222222222',
            'review-iqd-v1-2026-08-21', NULL, 'Sara', 'work-h-0',
        ...
        INSERT INTO human_decision_effect_events VALUES (
            1, 864, 'work-h-0', 'Sara', 'couch', 'accept', 4
        );
...
def test_final_playback_proof_uses_ledger_revision_not_one_second_timestamps() -> None:
    ...
    _insert_certificate_receipt(connection, reviewer="sara")
...
def test_final_playback_proof_requires_one_ledger_row_across_all_policies() -> None:
    ...
    connection.execute(
        """INSERT INTO review_compensation_ledger VALUES
             (864, 'another-policy', 4, 1000, 'Sara', 'work-h-0', 'couch',
              'reviewer-work-v1:4:sara:audio-segment-v1:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa:0:1000',
              'audio_content_hash+source_span')"""
    )
...
def test_all_four_hidden_actions_must_have_playback_evidence_too() -> None:
    found = issues(
        playback_failures=["event 884 (couch_spot_check, Sara) lacks exact playback evidence"],
        playback_checked=24,
    )
