...
FOCUS_IDS = PILOT_EVENT_IDS | {f"focus-filler-{index}" for index in range(8250)}
FOCUS = focus_evidence(FOCUS_IDS)
CONTENT_HASH = "a" * 64


def policy() -> ReviewPilotPolicy:
    return ReviewPilotPolicy(863, 20, {"Hawzhin": 10, "Pavel": 10})


def perfect_state() -> HiddenPilotState:
    grants = {
        "Hawzhin": {"hidden-h-1", "hidden-h-2"},
        "Pavel": {"hidden-p-1", "hidden-p-2"},
    }
    return HiddenPilotState(
        ...
        skipped_keys={"Hawzhin": set(), "Pavel": set()},
        unresolved_keys={"Hawzhin": set(), "Pavel": set()},
        corpus_actions={"Hawzhin": 10, "Pavel": 10},
        hidden_actions={"Hawzhin": 2, "Pavel": 2},
    )
...
def perfect_events() -> list[gate.ReviewEventEvidence]:
    ...
    event_id = 864
    for reviewer, prefix in (("Hawzhin", "h"), ("Pavel", "p")):
...
def perfect_compensation() -> dict[str, object]:
    return {
        ...
        "focusIds": 8274,
        "canonicalFocusRows": 8274,
        "uniqueCanonicalFocusWorkIds": 8274,
...
def test_zero_of_24_is_pending_not_a_capacity_green_certificate() -> None:
    empty = HiddenPilotState(
        policy_sha256=policy_sha256(policy()),
        grants={"Hawzhin": set(), "Pavel": set()},
        session_keys={"Hawzhin": set(), "Pavel": set()},
        completed_keys={"Hawzhin": set(), "Pavel": set()},
        skipped_keys={"Hawzhin": set(), "Pavel": set()},
        unresolved_keys={"Hawzhin": set(), "Pavel": set()},
        corpus_actions={"Hawzhin": 0, "Pavel": 0},
        hidden_actions={"Hawzhin": 0, "Pavel": 0},
    )
...
def test_19_corpus_actions_cannot_clear_the_gate() -> None:
    ...
    assert any("Hawzhin has 9/10 corpus" in item for item in found)
...
def test_bad_or_missing_hidden_result_is_red() -> None:
    ...
    quality[("Hawzhin", "hidden-h-1")] = (0, 0.5)
...
def test_hidden_quality_is_rederived_without_numeric_coercion_or_tolerance() -> None:
    for assignment, expected_error in (
        ...
        connection.execute(
            f"UPDATE spot_checks SET {assignment} WHERE segment_id = 'hidden-h-1' AND reviewer = 'Hawzhin'"
        )
        quality, errors = gate.read_hidden_quality(connection, perfect_state())
        connection.close()
        assert ("Hawzhin", "hidden-h-1") not in quality
...
def _playback_certificate_connection() -> sqlite3.Connection:
    ...
    connection.executescript(
        ...
        INSERT INTO review_compensation_ledger VALUES
            (864, 'review-iqd-v1-2026-08-21', 4, 1000, 'Hawzhin', 'work-h-0', 'couch',
...
def _manifest_connection() -> sqlite3.Connection:
    ...
    connection.executescript(
        ...
        INSERT INTO review_events VALUES (
            864, 'work-h-0', 'Hawzhin', 'accept', 'couch',
...
def test_final_playback_proof_uses_ledger_revision_not_one_second_timestamps() -> None:
    ...
    _insert_certificate_receipt(connection, reviewer="hawzhin")
...
def test_final_playback_proof_requires_one_ledger_row_across_all_policies() -> None:
    ...
    connection.execute(
        """INSERT INTO review_compensation_ledger VALUES
             (864, 'another-policy', 4, 1000, 'Hawzhin', 'work-h-0', 'couch',
              'reviewer-work-v1:7:hawzhin:audio-segment-v1:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa:0:1000',
              'audio_content_hash+source_span')"""
    )
...
def test_all_four_hidden_actions_must_have_playback_evidence_too() -> None:
    found = issues(
        playback_failures=["event 884 (couch_spot_check, Hawzhin) lacks exact playback evidence"],
        playback_checked=24,
    )
