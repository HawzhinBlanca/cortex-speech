...
def seed_profile(root: Path, *, schema_version: int = 59, policy: bool = True) -> None:
    ...
    if policy:
        (root / "review_pilot_policy.json").write_text(
            ...
            '"reviewers":[{"name":"Sara","max_corpus_actions":10},'
...
def test_schema59_hidden_keys_require_exact_policy_binding_and_schema58_stays_valid() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        connection = sqlite3.connect(data / snapshot.DB_FILE)
        connection.execute(
            "INSERT INTO review_pilot_hidden_keys VALUES(?, 0, 'Sara', 'hidden-1')",
            (policy_sha,),
        )
        connection.commit()
        connection.close()
    ...
    mismatches = (
        ("0" * 64, 0, "Sara", "disagrees with the active policy SHA/baseline"),
        (None, 1, "Sara", "disagrees with the active policy SHA/baseline"),
        (None, 0, "Nasrin", "unauthorized reviewer"),
    )
...
def test_active_snapshot_requires_completed_and_live_session_hidden_keys_to_be_durable() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        session = {
            ...
            "pilot_policy": policy,
            "pilot_spot_checks": [["remembered-hidden", "Sara"]],
        }
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        connection = sqlite3.connect(data / snapshot.DB_FILE)
        connection.execute(
            "INSERT INTO review_events VALUES(1, 'completed-hidden', 'Hemn', 'accept', 'couch_spot_check')"
        )
        connection.commit()
        connection.close()
        try:
...
def test_schema59_hidden_history_survives_policy_lifecycle_and_caps_each_namespace() -> None:
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        connection.executemany(
            ...
            [
                (active_sha, 0, "Sara", "active-hidden"),
