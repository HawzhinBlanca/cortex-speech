...
# Reviewer identity is operator data, never tracked code (the same rule that keeps the live roster in
# ``<data_dir>/reviewer_dialects.json``).  Tracked code ships fictional reviewers so every offline
# gate and fixture is reproducible on a machine with no data directory; an operator points the gates
# at the real pilot pair with ``CORTEX_PILOT_REVIEWERS="First,Second"``.  Anything that is not
# exactly two non-empty names falls back to the fictional pair, so the gates still fail closed
# against a fixed roster instead of whatever happens to be in the environment.
DEFAULT_PILOT_REVIEWERS = ("Sara", "Hemn")


def _configured_pilot_reviewers() -> tuple[str, ...]:
    names = tuple(part.strip() for part in os.environ.get("CORTEX_PILOT_REVIEWERS", "").split(",") if part.strip())
    return names if len(names) == len(DEFAULT_PILOT_REVIEWERS) else DEFAULT_PILOT_REVIEWERS


PILOT_REVIEWERS = _configured_pilot_reviewers()
