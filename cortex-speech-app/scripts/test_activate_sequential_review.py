#!/usr/bin/env python3
"""Regression tests for the fail-closed single-reviewer first-pass transition."""
...
def seed(root: Path, *, foreign_reviewer: bool = False) -> tuple[Path, dict[str, object]]:
    ...
    conn.executescript(
        ...
        INSERT INTO review_events VALUES(864, 'work-a', 'Sara', 'edit', 'couch');
        INSERT INTO review_events VALUES(865, 'hidden-a', 'Sara', 'accept', 'couch_spot_check');
        """
    )
    if foreign_reviewer:
        conn.execute("UPDATE review_events SET reviewer='Hemn' WHERE id=864")
    conn.executemany(
    ...
    session: dict[str, object] = {
        "reviewers": {"protected-sara": "Sara", "protected-hemn": "Hemn"},
        "db_path": str(db_path),
        "spot_checks": [["hidden-a", "Sara"], ["hidden-b", "Hemn"]],
        "pilot_spot_checks": [["hidden-a", "Sara"], ["hidden-b", "Hemn"]],
        "sessions": [
...
def test_activation_preserves_history_and_token_but_retires_every_pilot_surface() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        assert session["reviewers"] == {"protected-sara": REVIEWER}
...
def test_cas_and_foreign_reviewer_fail_before_any_live_mutation() -> None:
    for foreign_reviewer, expected_error in ((False, "CAS mismatch"), (True, "single-reviewer takeover")):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
