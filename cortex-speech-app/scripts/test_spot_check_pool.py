...
def test_pilot_progress_and_hidden_capacity_derive_from_the_enforced_cap() -> None:
    ...
    insert_v60_event(conn, 101, "work-h-1", "Hawzhin", "accept", "couch")
    insert_v60_event(conn, 102, "work-h-2", "Hawzhin", "skip", "couch")
    insert_v60_event(conn, 103, "hidden-p-1", "Pavel", "edit", "couch_spot_check")
    insert_v60_event(conn, 104, "work-p-1", "Pavel", "reject", "couch")
...
def test_pilot_progress_never_refunds_a_skip_shadowed_by_a_later_decision() -> None:
    ...
    insert_v60_event(conn, 1, "same-work", "Hawzhin", "skip", "couch")
    insert_v60_event(conn, 2, "same-work", "Hawzhin", "accept", "couch")
    total, progress = pilot_progress(conn, policy)
    assert total == 2 and progress["Hawzhin"] == 2
    conn.close()
...
def test_pilot_hidden_budget_is_db_authoritative_and_session_is_only_a_subset() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        insert_v60_event(conn, 101, "h1", "Hawzhin", "accept", "couch_spot_check")
        insert_v60_event(conn, 102, "p1", "Pavel", "skip", "couch_spot_check")
        conn.executemany(
        ...
        insert_v60_event(conn, 102, "p1", "Pavel", "skip", "couch")
        conn.commit()
        ...
        insert_v60_event(conn, 125, "work-too-many", "Hawzhin", "skip", "couch")
        conn.commit()
        try:
...
def test_pilot_history_with_an_unauthorized_reviewer_fails_closed() -> None:
    ...
    conn = sqlite3.connect(":memory:")
    create_v60_pilot_history(conn, 0)
    insert_v60_event(conn, 1, "work-r", "Rubar", "accept", "couch")
    try:
...
def test_ten_direct_actions_with_zero_checks_are_red_until_exact_two_results_exist() -> None:
    ...
    for index in range(1, 11):
        insert_v60_event(conn, index, f"work-{index}", "Hawzhin", "accept", "couch")
    total, progress = pilot_progress(conn, policy)
...
def test_hidden_skip_consumes_qc_slot_but_can_never_certify() -> None:
    ...
    insert_v60_event(conn, 1, "hidden-skip", "Hawzhin", "skip", "couch_spot_check")
