...
def test_capacity_is_per_reviewer_focus_dialect_and_prior_score() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert focused == {"Karwan": 0, "Sara": 1}
        ...
        scored = available_keys_by_reviewer(
            ...
            focus={"haw"},
            candidates=candidates,
            already_scored={("haw", "sara")},
            dialect_table=table,
        )
        assert scored == {"Sara": 0}, "a previously answered key must not count as fresh capacity"
...
def test_controlled_pilot_policy_is_exactly_two_reviewers_ten_each_twenty_total() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        valid = {
            ...
            "reviewers": [
                {"name": "Sara", "max_corpus_actions": 10},
                {"name": "Hemn", "max_corpus_actions": 10},
            ],
        }
...
def test_pilot_progress_and_hidden_capacity_derive_from_the_enforced_cap() -> None:
    policy = ReviewPilotPolicy(100, 20, {"Sara": 10, "Hemn": 10})
    conn = sqlite3.connect(":memory:")
    create_v60_pilot_history(conn, 100)
    insert_v60_event(conn, 101, "work-h-1", "Sara", "accept", "couch")
    insert_v60_event(conn, 102, "work-h-2", "Sara", "skip", "couch")
    insert_v60_event(conn, 103, "hidden-p-1", "Hemn", "edit", "couch_spot_check")
    insert_v60_event(conn, 104, "work-p-1", "Hemn", "reject", "couch")
    total, progress = pilot_progress(conn, policy)
    assert total == 3 and progress == {"Sara": 2, "Hemn": 1}
    bounded = pilot_bounded_work_counts({"Sara": 8_215, "Hemn": 8_215}, policy, progress)
    assert bounded == {"Sara": 8, "Hemn": 9}
    ...
    assert pilot_required_fresh_keys(10, bounded["Sara"], 0, 25, 8) == 1
...
def test_pilot_progress_never_refunds_a_skip_shadowed_by_a_later_decision() -> None:
    policy = ReviewPilotPolicy(0, 20, {"Sara": 10, "Hemn": 10})
    conn = sqlite3.connect(":memory:")
    create_v60_pilot_history(conn, 0)
    insert_v60_event(conn, 1, "same-work", "Sara", "skip", "couch")
    insert_v60_event(conn, 2, "same-work", "Sara", "accept", "couch")
    total, progress = pilot_progress(conn, policy)
    assert total == 2 and progress["Sara"] == 2
    conn.close()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (root / "couch_session.json").write_text(
            json.dumps(
                {
                    "db_path": str(db_path),
                    "reviewers": {"token-h": "Sara", "token-p": "Hemn"},
                    "pilot_policy": raw_policy,
                    # A restart may lag the DB; the cache is allowed to be a strict subset.
                    "pilot_spot_checks": [["h1", "Sara"]],
                }
            ),
            encoding="utf-8",
        )
        grants, state = load_pilot_served_checks(root, policy, conn, db_path)
        assert grants == {"Sara": {"h1"}, "Hemn": {"p1"}}
        assert state.session_keys == {"Sara": {"h1"}, "Hemn": set()}

        insert_v60_event(conn, 101, "h1", "Sara", "accept", "couch_spot_check")
        insert_v60_event(conn, 102, "p1", "Hemn", "skip", "couch_spot_check")
        conn.executemany(
            "INSERT INTO spot_checks VALUES (?, ?, ?)",
            [("h1", "Sara", "accept"), ("p1", "Hemn", "skip")],
        )
        conn.commit()
        _grants, resolved = load_pilot_served_checks(root, policy, conn, db_path)
        assert resolved.completed_keys == {"Sara": {"h1"}, "Hemn": set()}
        assert resolved.skipped_keys == {"Sara": set(), "Hemn": {"p1"}}
        assert resolved.unresolved_keys == {"Sara": set(), "Hemn": set()}
        ...
        conn.execute("DELETE FROM spot_checks WHERE segment_id = 'p1' AND reviewer = 'Hemn'")
        insert_v60_event(conn, 102, "p1", "Hemn", "skip", "couch")
        conn.commit()
        _grants, legacy = load_pilot_served_checks(root, policy, conn, db_path)
        assert legacy.skipped_keys == {"Sara": set(), "Hemn": {"p1"}}
        ...
        conn.executemany(
            "INSERT INTO review_pilot_hidden_keys VALUES (?, 100, ?, ?)",
            [(digest, "Sara", "h2"), (digest, "Hemn", "p2")],
        )
        corpus_events = [
            (101 + index, f"work-h-{index}", "Sara", "accept", "couch")
            for index in range(10)
        ] + [
            (111 + index, f"work-p-{index}", "Hemn", "edit", "couch")
            for index in range(10)
        ]
        hidden_events = [
            (121, "h1", "Sara", "accept", "couch_spot_check"),
            (122, "h2", "Sara", "edit", "couch_spot_check"),
            (123, "p1", "Hemn", "reject", "couch_spot_check"),
            (124, "p2", "Hemn", "accept", "couch_spot_check"),
        ]
        ...
        insert_v60_event(conn, 125, "work-too-many", "Sara", "skip", "couch")
        conn.commit()
        try:
        ...
        broken["pilot_spot_checks"] = [["never-reserved", "Hemn"]]
...
def test_policy_digest_is_semantic_and_matches_the_rust_byte_contract() -> None:
    canonical = ReviewPilotPolicy(863, 20, {"Sara": 10, "Hemn": 10})
    reordered_and_recased = ReviewPilotPolicy(863, 20, {"hEmN": 10, "SARA": 10})
    # Golden vector over the tracked fictional roster: it pins the canonical serialisation (sorted,
    # lower-cased names; no whitespace drift), so any change to the byte contract still breaks here.
    assert policy_sha256(canonical) == "5df2c936a92b0009b6ff99bcc3c73c4a4eaaae5430c54e4f71f4aaf1e050a074"
...
def test_pilot_history_with_an_unauthorized_reviewer_fails_closed() -> None:
    ...
    conn = sqlite3.connect(":memory:")
    create_v60_pilot_history(conn, 0)
    insert_v60_event(conn, 1, "work-s", "Nasrin", "accept", "couch")
    try:
...
def test_ten_direct_actions_with_zero_checks_are_red_until_exact_two_results_exist() -> None:
    ...
    for index in range(1, 11):
        insert_v60_event(conn, index, f"work-{index}", "Sara", "accept", "couch")
    total, progress = pilot_progress(conn, policy)
    assert total == 10 and progress["Sara"] == 10
    served = {"Sara": set(), "Hemn": set()}
    ...
    served["Sara"] = {"key-1", "key-2"}
    conn.executemany(
        "INSERT INTO spot_checks VALUES (?, 'Sara', 1, 0.0)",
        [("key-1",), ("key-2",)],
    )
...
def test_hidden_skip_consumes_qc_slot_but_can_never_certify() -> None:
    ...
    insert_v60_event(conn, 1, "hidden-skip", "Sara", "skip", "couch_spot_check")
    conn.execute("INSERT INTO spot_checks VALUES ('hidden-skip', 'Sara', 0, 1.0)")
    total, progress = pilot_progress(conn, policy)
    assert total == 0 and progress["Sara"] == 0, "hidden QC must not consume corpus quota"
    issues = pilot_certification_issues(
        conn,
        policy,
        progress,
        {"Sara": {"hidden-skip"}, "Hemn": set()},
        8,
    )
