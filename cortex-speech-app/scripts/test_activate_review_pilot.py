...
def seed(root: Path, *, schema: int = 67) -> tuple[Path, dict[str, object]]:
    ...
    session: dict[str, object] = {
        ...
        "db_path": str(db_path),
        "spot_checks": [["r1", "Sara"], ["a1", "Hemn"], ["s1", "Nasrin"]],
        "pilot_spot_checks": [],
        "sessions": [
            {"token": "cookie-r", "reviewer": "Sara", "issued_unix": 1},
            {"token": "cookie-a", "reviewer": "Hemn", "issued_unix": 2},
            {"token": "cookie-s", "reviewer": "Nasrin", "issued_unix": 3},
        ],
    }
...
def test_activation_preserves_target_tokens_narrows_every_session_surface_and_backs_up() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        assert [entry["name"] for entry in policy["reviewers"]] == ["Hemn", "Sara"]
        ...
        assert {entry["reviewer"] for entry in session["sessions"]} == {"Sara", "Hemn"}
        assert {entry[1] for entry in session["spot_checks"]} == {"Sara", "Hemn"}
...
def test_first_activation_refuses_unnamespaced_legacy_pilot_hidden_keys() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        session["pilot_spot_checks"] = [["ambiguous-key", "Sara"]]
...
def test_pristine_roster_replacement_atomically_extends_the_exact_focus() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        old_policy["reviewers"] = [
            {"name": "Shilan", "max_corpus_actions": 10},
            {"name": "Dilan", "max_corpus_actions": 10},
        ]
        policy_path = root / POLICY_FILE
        policy_path.write_text(json.dumps(old_policy), encoding="utf-8")
        current_session = {
            "reviewers": {"old-h": "Shilan", "old-p": "Dilan"},
            "db_path": str(db_path),
            "spot_checks": [],
            "pilot_spot_checks": [],
            "sessions": [],
...
def test_existing_pilot_cannot_reset_its_baseline_after_any_durable_activity() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        conn = sqlite3.connect(db_path)
        conn.execute(
            "INSERT INTO review_events VALUES(864, 'corpus-work', 'Sara', 'accept', 'couch')"
        )
        conn.commit()
        conn.close()
...
def test_schema63_activation_imports_session_and_completed_hidden_keys_into_one_namespace() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        session["pilot_spot_checks"] = [
            ["hidden-r-session", "Sara"],
            ["hidden-a-session", "Hemn"],
        ]
        session_path.write_text(json.dumps(session), encoding="utf-8")
        conn = sqlite3.connect(db_path)
        conn.executemany(
            ...
            [
                (864, "hidden-r-completed", "Sara", "accept"),
                (865, "hidden-a-completed", "Hemn", "reject"),
            ],
        )
        conn.commit()
        conn.close()
        ...
        conn.close()
        assert rows == [
            (expected_digest, 863, "Hemn", "hidden-a-completed"),
            (expected_digest, 863, "Hemn", "hidden-a-session"),
            (expected_digest, 863, "Sara", "hidden-r-completed"),
            (expected_digest, 863, "Sara", "hidden-r-session"),
        ]
    ...
    for mutation, expected in (
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            if mutation == "over_quota":
                conn.executemany(
                    "INSERT INTO review_events VALUES(?, ?, 'Sara', 'accept', 'couch_spot_check')",
...
def test_non_restartable_or_duplicate_session_json_is_refused_before_revocation() -> None:
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        session_path.write_text(
            '{"reviewers":{"dpapi-secret":"Sara","dpapi-secret":"Hemn"},'
