...
CURRENT_SCHEMA = source_migrations(DEFAULT_MIGRATIONS)[-1][0]


def test_required_schema_tracks_the_append_only_rust_catalog() -> None:
    assert activator.REQUIRED_SCHEMA == CURRENT_SCHEMA
...
def seed(root: Path, *, schema: int = CURRENT_SCHEMA) -> tuple[Path, dict[str, object]]:
...
def test_activation_preserves_target_tokens_narrows_every_session_surface_and_backs_up() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        assert [entry["name"] for entry in policy["reviewers"]] == ["Aram", "Rezan"]
        ...
        assert {entry["reviewer"] for entry in session["sessions"]} == {"Rezan", "Aram"}
        assert {entry[1] for entry in session["spot_checks"]} == {"Rezan", "Aram"}
...
def test_first_activation_refuses_unnamespaced_legacy_pilot_hidden_keys() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        session["pilot_spot_checks"] = [["ambiguous-key", "Rezan"]]
...
def test_pristine_roster_replacement_atomically_extends_the_exact_focus() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        current_session = {
            "reviewers": {"old-h": "Hawzhin", "old-p": "Karwan"},
            "db_path": str(db_path),
            "spot_checks": [],
            "pilot_spot_checks": [],
            "sessions": [],
...
def test_existing_pilot_cannot_reset_its_baseline_after_any_durable_activity() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        conn = sqlite3.connect(db_path)
        conn.execute(
            "INSERT INTO review_events VALUES(864, 'corpus-work', 'Rezan', 'accept', 'couch')"
        )
        conn.commit()
        conn.close()
...
def test_schema63_activation_imports_session_and_completed_hidden_keys_into_one_namespace() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        session["pilot_spot_checks"] = [
            ["hidden-r-session", "Rezan"],
            ["hidden-a-session", "Aram"],
        ]
        session_path.write_text(json.dumps(session), encoding="utf-8")
        conn = sqlite3.connect(db_path)
        conn.executemany(
            ...
            [
                (864, "hidden-r-completed", "Rezan", "accept"),
                (865, "hidden-a-completed", "Aram", "reject"),
            ],
        )
        conn.commit()
        conn.close()
        ...
        conn.close()
        assert rows == [
            (expected_digest, 863, "Aram", "hidden-a-completed"),
            (expected_digest, 863, "Aram", "hidden-a-session"),
            (expected_digest, 863, "Rezan", "hidden-r-completed"),
            (expected_digest, 863, "Rezan", "hidden-r-session"),
        ]
    ...
    for mutation, expected in (
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            if mutation == "over_quota":
                conn.executemany(
                    "INSERT INTO review_events VALUES(?, ?, 'Rezan', 'accept', 'couch_spot_check')",
...
def test_schema_56_is_refused_before_any_activation_file_changes() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        try:
            ...
            assert f"schema 56/{CURRENT_SCHEMA}" in str(error)
        else:
...
def test_non_restartable_or_duplicate_session_json_is_refused_before_revocation() -> None:
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        session_path.write_text(
            '{"reviewers":{"dpapi-secret":"Rezan","dpapi-secret":"Aram"},'
