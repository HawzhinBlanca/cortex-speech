...
def test_wrong_dialect_gate_reads_current_attribution_not_superseded_history() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            # Historical Hemn edit, now superseded by an unattributed legacy reject: it is neither
            # current attribution nor downstream training data and must not accuse Hemn today.
            con.execute(
                "INSERT INTO speech_segments VALUES (?, ?, 1, 'reject', NULL)",
                ("superseded", HAWLERI_CLIP[0]),
            )
            con.execute("INSERT INTO review_events VALUES ('Hemn', 'superseded', 'edit')")
            ...
            con.execute(
                "INSERT INTO speech_segments VALUES (?, ?, 1, 'accept', 'Hemn')",
                ("current-accept", HAWLERI_CLIP[0]),
            )
            con.execute(
                "INSERT INTO speech_segments VALUES (?, ?, 1, 'reject', 'Hemn')",
                ("current-reject", HAWLERI_CLIP[0]),
            )
            con.execute(
                "INSERT INTO speech_segments VALUES (?, ?, 1, 'edit', 'Shilan')",
...
def test_the_real_incident_a_sorani_only_reviewer_with_no_sorani_clips_fails() -> None:
    problems, _ = evaluate_queues(
        ...
        roster={"Karwan": ["sorani"], "Sara": ["hawleri", "sorani"]},
        clips=[HAWLERI_CLIP],
        table=TABLE,
    )
    assert len(problems) == 1, problems
    assert "Karwan" in problems[0]
...
def test_an_empty_library_fails_for_everyone_because_nobody_can_work() -> None:
    problems, _ = evaluate_queues(reviewers=["Karwan"], roster={"Karwan": ["sorani"]}, clips=[], table=TABLE)
...
def test_flexible_pool_counts_are_reviewer_specific_and_zero_fails() -> None:
    problems, warnings = evaluate_pool_queues({"Hemn": 120, "Sara": 0}, warn_below=100)
    assert len(problems) == 1 and "Sara" in problems[0] and "ZERO" in problems[0]
    assert warnings == []
    problems, warnings = evaluate_pool_queues({"Hemn": 99, "Sara": 101}, warn_below=100)
    assert problems == []
    assert len(warnings) == 1 and "Hemn" in warnings[0] and "99" in warnings[0]
...
def test_unmapped_clips_fail_closed_and_are_not_counted_as_work() -> None:
    ...
    problems, _ = evaluate_queues(
        reviewers=["Karwan"], roster={"Karwan": ["sorani"]}, clips=[unmapped], table=TABLE
    )
    assert len(problems) == 1
...
def test_a_comment_key_does_not_empty_the_roster() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        (Path(tmp) / "reviewer_dialects.json").write_text(
            json.dumps({"_comment": "how to edit", "Karwan": ["sorani"]}), encoding="utf-8"
        )
        roster = load_roster(Path(tmp))
    assert roster == {"Karwan": ["sorani"]}, roster
...
def test_a_broken_policy_file_fails_the_gate_instead_of_unrestricting_everyone() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (Path(tmp) / "reviewer_dialects.json").write_text(json.dumps({"Karwan": "sorani"}), encoding="utf-8")
        try:
            load_roster(Path(tmp))
            raise AssertionError("a typo'd roster entry must raise PolicyBroken")
        except PolicyBroken as e:
            assert "Karwan" in str(e), e
...
def test_a_roster_key_binds_across_case_and_whitespace_and_duplicates_are_broken() -> None:
    # 2026-08-20 hunt: the exact-match lookup let an orphaned key ("karwan " vs live "Karwan") load
    ...
    with tempfile.TemporaryDirectory() as tmp:
        (Path(tmp) / "reviewer_dialects.json").write_text(json.dumps({"karwan ": ["sorani"]}), encoding="utf-8")
        roster = load_roster(Path(tmp))
        assert allowed_for(roster, "Karwan") == ["sorani"], "the key must bind its reviewer across case/space"
        assert allowed_for(roster, "Sara") is None
    with tempfile.TemporaryDirectory() as tmp:
        (Path(tmp) / "reviewer_dialects.json").write_text(
            json.dumps({"Karwan": ["sorani"], "karwan": ["hawleri"]}), encoding="utf-8"
        )
        try:
            load_roster(Path(tmp))
...
def test_roster_values_are_normalized_and_unknown_or_empty_dialects_are_broken() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        (Path(tmp) / "reviewer_dialects.json").write_text(
            json.dumps({"Karwan": [" Sorani ", "SORANI", "Hawleri"]}), encoding="utf-8"
        )
        assert load_roster(Path(tmp)) == {"Karwan": ["sorani", "hawleri"]}

    for invalid in ({"Karwan": ["soranii"]}, {"Karwan": []}):
        with tempfile.TemporaryDirectory() as tmp:
