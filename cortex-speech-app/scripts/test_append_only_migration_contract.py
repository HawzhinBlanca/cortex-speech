...
LOCKED_VERSIONS = [66, 67, 68, 69, 70, 71]
...
def test_append_only_migrations_66_through_71_match_individual_source_hashes() -> None:
...
def test_future_append_does_not_redefine_locked_migration_blocks() -> None:
    ...
    assert migrations[-1].version == 72


def main() -> None:
    test_append_only_migrations_66_through_71_match_individual_source_hashes()
    ...
    print(f"append-only migration source blocks 66-71 are immutable: {hashes}")
