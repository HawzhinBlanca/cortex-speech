...
fn restore_authority_requires_exact_legacy_baseline_and_append_only_desktop_journal_prefix() {
    ...
    assert_eq!(crate::migrations::rollback(&baseline, 4).unwrap(), vec![72, 71, 70, 69]);
    append_decision(&baseline, "legacy-baseline-decision");
    assert_eq!(crate::migrations::run_migrations(&baseline).unwrap(), vec![69, 70, 71, 72]);
...
fn delayed_v68_clones_migrate_to_one_deterministic_legacy_barrier_and_remain_restore_compatible() {
    ...
    let floor = make_file_db(&floor_path);
    assert_eq!(crate::migrations::rollback(&floor, 4).unwrap(), vec![72, 71, 70, 69]);
    ...
    assert_eq!(crate::migrations::run_migrations(&floor).unwrap(), vec![69, 70, 71, 72]);
    std::thread::sleep(std::time::Duration::from_millis(1_100));
    let target = Database::open(target_path.to_string_lossy().as_ref()).unwrap();
    assert_eq!(crate::migrations::run_migrations(&target).unwrap(), vec![69, 70, 71, 72]);
