...
fn detached_read_snapshot_cannot_mutate_its_source_database() {
    ...
    assert_eq!(crate::migrations::validate_applied_history(snapshot.connection()).unwrap(), 63);
...
fn restore_stages_pending_migrations_and_foreign_keys_before_overwriting_live_data() {
    ...
    let migration_fail_path = tmp.path().join("migration-fail.db");
    {
        ...
        assert_eq!(crate::migrations::rollback(&candidate, 6).unwrap(), vec![63, 62, 61, 60, 59, 58]);
        candidate
            .connection()
            .execute_batch(
...
#[test]
fn schema_v58_upgrades_through_v63_without_reinterpreting_live_baseline_863() {
    let db = make_db();
    assert_eq!(crate::migrations::rollback(&db, 5).unwrap(), vec![63, 62, 61, 60, 59]);
    db.connection()
        .execute(
            "INSERT INTO review_events
        ...
        .unwrap();
    assert_eq!(crate::migrations::run_migrations(&db).unwrap(), vec![59, 60, 61, 62, 63]);
