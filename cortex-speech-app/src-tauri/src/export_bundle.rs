...
pub fn current_canonical_pcm_blake3(audio_path: &Path) -> AppResult<String> {
...
fn export_dataset_bundle_inner(
    ...
    // Per-speaker composition + dominant-speaker flag in the HUMAN-readable card — it lived only in
    // dataset.json, so the stated skew warning never reached a card reader (true-10 audit
    // 2026-07-09).
    let composition_md = export::composition_markdown(&export::compute_composition(&segments));
...
fn build_training_grade_details(
    ...
    for segment in segments {
        ...
        details.push(TrainingGradeSegmentDetail {
            ...
            agreement_score: segment.agreement_score,
