...
mod tests {
    ...
    fn claim_probe_helper_is_exact_read_only_and_detects_cookie_amnesia() {
        ...
        let pairing = HashMap::from([
            ("pair-chiman".to_string(), "Chiman".to_string()),
        ...
        let mut sessions: HashMap<String, (String, SystemTime)> = (0..MAX_SESSIONS_PER_REVIEWER)
            .map(|index| (format!("cookie-{index}"), ("Chiman".to_string(), issued)))
            .collect();
        sessions.insert("expired-cookie".to_string(), ("Chiman".to_string(), expired));
        let spot_checks = HashSet::from([
            ("hidden-h".to_string(), "Chiman".to_string()),
        ...
        for _ in 0..9 {
            let reply = api_claim_probe(br#"{"token":"pair-chiman"}"#, &state);
            assert_eq!(reply.0, 200);
            ...
            assert_eq!(body["reviewer"], "Chiman");
            ...
            assert!(!body_text.contains("pair-chiman"), "the credential must never be echoed");
        ...
        for body in [
            ...
            br#"{}"#,
            br#"{"token":42}"#,
            br#"{"token":"pair-chiman","extra":true}"#,
            br#"{"token":"pair-chiman","token":"pair-karwan"}"#,
            br#"{"token":"unknown"}"#,
            br#"{"token":"cookie-0"}"#,
            oversized.as_slice(),
        ] {
        ...
        // evicting anything as a side effect of observation.
        {
            ...
            guard.reviewers.insert("cookie-live-only".to_string(), "Chiman".to_string());
    ...
    fn claim_probe_http_route_precedes_cookie_pruning_and_never_sets_a_cookie() {
        ...
        let (_db, db_path) = test_db(tmp.path());
        let policy = probe_test_policy();
        let pairing = HashMap::from([("pair-chiman".to_string(), "Chiman".to_string())]);
        let issued = UNIX_EPOCH
            + Duration::from_secs(SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs().saturating_sub(60));
        let expired = SystemTime::now() - COUCH_SESSION_TTL - Duration::from_secs(60);
        let sessions = HashMap::from([
            ("cookie-live".to_string(), ("Chiman".to_string(), issued)),
            ("cookie-expired".to_string(), ("Chiman".to_string(), expired)),
        ]);
        ...
        let after_page = agent
            ...
            .send_json(ureq::json!({"token": "pair-chiman"}))
    ...
    fn flexible_pool_is_voice_named_coverage_first_and_preserves_the_first_review() {
        ...
        assert_eq!(queue["items"][1]["text"], "champion reviewed draft", "later reviews stay blind to Rezan's answer");
    ...
    fn controlled_pilot_hidden_key_budget_is_two_and_survives_refills_without_growth() {
        ...
        assert_eq!(pilot_spot_check_quota(&policy, "Chiman").unwrap(), 2);
    ...
    fn corpus_cap_serves_exactly_two_hidden_only_catchup_checks_and_never_a_third() {
        ...
        for timestamp in 1..=10 {
            db.record_review_event("pilot-work", "Chiman", "skip", "couch", timestamp).unwrap();
        }
        std::fs::write(
            dir.path().join(crate::review_pilot::REVIEW_PILOT_FILE),
            r#"{
        ...
        let state = Mutex::new(CouchState {
            ...
            pairing_codes: HashMap::from([
                ("chiman-pair".to_string(), "Chiman".to_string()),
        ...
        assert_eq!(api_decision(&db, direct.to_string().as_bytes(), "Chiman", &state).0, 409);
        assert_eq!(
        ...
        let fetch = || {
            let (code, _, body, ..) = api_queue(&db, "Chiman", &state);
            assert_eq!(code, 200);
        ...
        for item in reload["items"].as_array().unwrap() {
            ...
            assert_eq!(api_decision(&db, body.to_string().as_bytes(), "Chiman", &state).0, 200);
        }
        ...
        let hidden_skip_source: String = db
            .connection()
            .query_row(
                "SELECT source FROM review_events WHERE segment_id='pilot-key-2' AND reviewer='Chiman' ORDER BY id DESC LIMIT 1",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn pilot_uses_sqlite_authority_when_the_session_mirror_is_lost() {
        ...
        for timestamp in 1..=10 {
            db.record_review_event("persist-work", "Chiman", "skip", "couch", timestamp).unwrap();
        }
        std::fs::write(
            dir.path().join(crate::review_pilot::REVIEW_PILOT_FILE),
            r#"{
        ...
        let (code, _, first_body, ..) = api_queue(&db, "Chiman", &failed_state);
        assert_eq!(code, 200);
        ...
        assert_eq!(
            db.review_pilot_hidden_keys(&policy_sha256, policy.after_review_event_id, "Chiman").unwrap(),
            first_ids,
        ...
        let (code, _, body, ..) = api_queue(&db, "Chiman", &restarted);
        assert_eq!(code, 200);
        let payload: serde_json::Value = serde_json::from_slice(&body).unwrap();
    ...
    fn a_desktop_decision_is_never_silently_overwritten_by_the_phone() {
        ...
        let held = queue_ids(&db, "Chiman", &state);
        ...
        let (code, _, msg, ..) = api_decision(&db, body.to_string().as_bytes(), "Chiman", &state);
    ...
    fn reloading_mid_batch_returns_the_remainder_and_never_a_decided_clip() {
        ...
        let port = server.server_addr().to_ip().unwrap().port();
        let state = Arc::new(Mutex::new(CouchState {
            reviewers: [("tok".to_string(), "Chiman".to_string())].into_iter().collect(),
            ..CouchState::default()
        }));
        let shutdown = Arc::new(AtomicBool::new(false));
        ...
        for item in first.iter().take(5) {
            let id = item["id"].as_str().unwrap();
            let playback_receipt_id = fixture_policy4_receipt(&proof_db, "Chiman", "tok", id, &item["rowVersion"]);
            let body = serde_json::json!({
                "operationId": uuid::Uuid::new_v4().to_string(),
    ...
    fn a_mid_session_server_restart_loses_no_work_and_double_decides_nothing() {
        ...
        let submit = |port: u16, id: &str, row_version: &serde_json::Value| -> u16 {
            ...
            let playback_receipt_id = {
                ...
                receipts
                    .entry(id.to_string())
                    .or_insert_with(|| fixture_policy4_receipt(&proof_db, "Chiman", "tok", id, row_version))
                    .clone()
            };
            let body = serde_json::json!({
                "operationId": operation_id,
        ...
        for id in &decided {
            ...
            assert_eq!(row.reviewed_by.as_deref(), Some("Chiman"), "{id} lost its attribution");
        }
    ...
    fn one_reviewer_can_drain_a_backlog_larger_than_a_batch_to_genuine_zero() {
        ...
        let port = server.server_addr().to_ip().unwrap().port();
        let state = Arc::new(Mutex::new(CouchState {
            reviewers: [("tok-solo".to_string(), "Chiman".to_string())].into_iter().collect(),
            ..CouchState::default()
        }));
        let shutdown = Arc::new(AtomicBool::new(false));
        ...
        loop {
            ...
            for item in items {
                ...
                let playback_receipt_id =
                    fixture_policy4_receipt(&proof_db, "Chiman", "tok-solo", &id, &item["rowVersion"]);
                let body = serde_json::json!({
                    "operationId": uuid::Uuid::new_v4().to_string(),
                    "playbackReceiptId": playback_receipt_id,
                    "id": id,
    ...
    fn pilot_cap_blocks_work_audio_but_preserves_only_outstanding_pilot_checks() {
        ...
        for timestamp in 1..=10 {
            db.record_review_event("audio-pilot-work", "Chiman", "skip", "couch", timestamp).unwrap();
        }
        std::fs::write(
        ...
        let state = Mutex::new(CouchState {
            session_store: Some((tmp.path().to_path_buf(), db_path)),
            pairing_codes: HashMap::from([
                ("chiman-pair".into(), "Chiman".into()),
                ("karwan-pair".into(), "Karwan".into()),
            ]),
            pilot_policy: Some(policy),
            leases: HashMap::from([("audio-pilot-work".into(), ("Chiman".into(), Instant::now()))]),
            served_work: HashSet::from([("audio-pilot-work".into(), "Chiman".into())]),
            spot_checks: HashSet::from([
                ("audio-pilot-key".into(), "Chiman".into()),
                ("audio-old-key".into(), "Chiman".into()),
            ]),
            pilot_spot_checks: HashSet::from([("audio-pilot-key".into(), "Chiman".into())]),
            ..CouchState::default()
        });
        ...
        assert_eq!(
            api_audio(&db, "audio-pilot-work", "Chiman", &state, None, None).0,
            403,
            "the eleventh corpus action cannot be enabled by fetching its audio"
        );
        assert_eq!(
            api_audio(&db, "audio-old-key", "Chiman", &state, None, None).0,
            403,
            "a hidden key from outside the bound pilot is not a pilot audio grant"
        );
        assert_eq!(
            api_audio(&db, "audio-pilot-key", "Chiman", &state, None, None).0,
            200,
        ...
        db.connection()
            ...
            .unwrap();
        db.record_spot_check("audio-pilot-key", "Chiman", "edit", "ڕاستی پایلۆت", "ڕاستی پایلۆت").unwrap();
        assert_eq!(
            api_audio(&db, "audio-pilot-key", "Chiman", &state, None, None).0,
            403,
