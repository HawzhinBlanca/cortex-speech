...
mod tests {
    ...
    fn pool_orders_least_covered_and_keeps_second_review_append_only() {
        ...
        assert_eq!(pending_segment_ids(&db, &pool, "Aram", None).unwrap(), vec!["second", "first"]);
    ...
    fn two_exact_outcomes_resolve_and_stop_further_serving() {
        ...
        assert!(pending_segment_ids(&db, &pool, "Zana", None).unwrap().is_empty());
    }
    ...
    fn disagreement_gets_one_blinded_third_review_then_resolves_by_matching_pair() {
        ...
        assert_eq!(pending_segment_ids(&db, &pool, "Zana", None).unwrap(), vec!["clip"]);
        decide(&db, &pool, "Zana", "دەقی یەکەم", "123e4567-e89b-42d3-a456-426614174053", 2);
        let row = segment_resolutions(&db, None).unwrap().pop().unwrap();
        assert_eq!(row.status, "resolved");
        assert_eq!(row.final_transcript.as_deref(), Some("دەقی یەکەم"));
        assert_eq!(row.agreeing_reviewers, vec!["Rezan", "Zana"]);
    ...
    fn three_distinct_outcomes_require_owner_and_owner_ruling_is_evidence_bound() {
        ...
        let third_id = decide(&db, &pool, "Zana", "دەقی سێیەم", "123e4567-e89b-42d3-a456-426614174055", 2);
        ...
        reverse_decision(&db, &pool, third_id, "Zana", "123e4567-e89b-42d3-a456-426614174057", 4).unwrap();
