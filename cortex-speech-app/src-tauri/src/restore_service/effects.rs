...
mod tests {
    ...
    fn first_post_v60_decision_must_start_from_the_exact_legacy_reviewed_state() {
        ...
        let db = Database::open(":memory:").unwrap();
        db.initialize().unwrap();
        assert_eq!(crate::migrations::rollback(&db, 12).unwrap(), vec![71, 70, 69, 68, 67, 66, 65, 64, 63, 62, 61, 60]);
        ...
        assert_eq!(
            ...
            vec![60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71]
        );
    ...
    fn every_reachable_legacy_decision_origin_field_is_refused() {
        let migrated_reviewed_db = |id: &str| {
            ...
            db.initialize().unwrap();
            assert_eq!(
                crate::migrations::rollback(&db, 12).unwrap(),
