...
fn big_response_reader() -> Reader {
    ...
    MeteredReader { inner: Cursor::new(big_body), position: Arc::new(AtomicUsize::new(0)) }
}

fn identity_served(r: &mut Reader) -> tiny_http_fork::Response<&mut Reader> {
    let body_len = r.inner.get_ref().len();
    tiny_http_fork::Response::empty(200).with_chunked_threshold(std::usize::MAX).with_data(r, Some(body_len))
}
...
fn responding_to_closed_client() {
    ...
    request.respond(identity_served(&mut reader)).expect("Successful");
