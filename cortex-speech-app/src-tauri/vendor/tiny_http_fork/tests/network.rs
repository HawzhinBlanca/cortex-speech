...
fn responses_reordered() {
    ...
    thread::spawn(move || {
        let rq1 = server.recv().unwrap().unwrap();
        let rq2 = server.recv().unwrap().unwrap();

        thread::spawn(move || {
            rq2.respond(tiny_http_fork::Response::from_string("second request".to_owned())).unwrap();
        });

        thread::sleep(Duration::from_millis(100));

        thread::spawn(move || {
            rq1.respond(tiny_http_fork::Response::from_string("first request".to_owned())).unwrap();
        });
    });
...
fn no_transfer_encoding_on_204() {
    ...
    thread::spawn(move || {
        let rq = server.recv().unwrap().unwrap();
