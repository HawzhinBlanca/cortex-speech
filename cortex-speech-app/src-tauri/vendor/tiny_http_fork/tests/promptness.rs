...
mod prompt_pipelining {
    ...
    fn assert_requests_parsed_promptly(
        ...
        server_ip = if server_ip.ip().is_unspecified() == true {
        ...
        spawn(move || {
            for _ in 0..req_cnt {
                let mut req = server.recv().unwrap().unwrap();
                ...
                spawn(move || req.respond(Response::empty(200).with_data(resp_body, Some(resp_body.len))));
            }
        ...
        svr_rcv.recv_timeout(timeout).expect("Server did not finish reading pipelined requests quickly enough");
    }
...
mod prompt_responses {
    ...
    fn assert_responds_promptly(timeout: Duration, req_writer: impl FnOnce(&mut dyn Write) + Send + 'static) {
        // BSD fix, it does not want to connect to 0.0.0.0:port
    ...
    fn content_length_http11() {
        ...
        let milis = 300;
        assert_responds_promptly(Duration::from_millis(milis), move |wr| {
            write!(wr, "GET / HTTP/1.1\r\n").unwrap();
