...
fn home_page(port: u16) -> tiny_http_fork::Response<Cursor<Vec<u8>>> {
    tiny_http_fork::Response::from_string(format!(
        "
    ...
    .with_header("Content-type: text/html".parse::<tiny_http_fork::Header>().unwrap())
}
...
fn convert_key(input: &str) -> String {
    ...
    sha1.finalize().to_base64(Config { char_set: Standard, pad: true, line_length: None, newline: Newline::LF })
}
...
fn main() {
    ...
    for request in server.incoming_requests() {
        ...
        spawn(move || {
            ...
            match request.headers().get("Upgrade").and_then(
                |hdr| {
                    if hdr.as_str() == "websocket" { Some(hdr) } else { None }
                },
            ) {
                None => {
            ...
            let key = match request.headers().get("Sec-WebSocket-Key").map(|h| h.clone()) {
                None => {
                    let response = tiny_http_fork::Response::new_empty(tiny_http_fork::StatusCode(400));
            ...
            let response = tiny_http_fork::Response::new_empty(tiny_http_fork::StatusCode(101))
                .with_header("Upgrade: websocket".parse::<tiny_http_fork::Header>().unwrap())
                .with_header("Connection: Upgrade".parse::<tiny_http_fork::Header>().unwrap())
                .with_header("Sec-WebSocket-Protocol: ping".parse::<tiny_http_fork::Header>().unwrap())
                .with_header(
                    format!("Sec-WebSocket-Accept: {}", convert_key(key.as_str()))
                        .parse::<tiny_http_fork::Header>()
                        .unwrap(),
                );

            //
            let mut stream = request.upgrade("websocket", response).unwrap();
