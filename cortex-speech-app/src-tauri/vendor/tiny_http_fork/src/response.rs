use crate::Headers;
use crate::common::{
    HEADER_CONNECTION, HEADER_CONTENT_TYPE, HEADER_SERVER, HEADER_TRANSFER_ENCODING, HTTPVersion, Header, HeaderError,
    StatusCode,
};
...
impl FromStr for TransferEncoding {
    ...
    fn from_str(input: &str) -> Result<TransferEncoding, ()> {
        if input.eq_ignore_ascii_case("identity") == true {
            Ok(TransferEncoding::Identity)
        } else if input.eq_ignore_ascii_case("chunked") == true {
    ...
    W: Write,
{
    ...
    // writing headers
    headers.write_headers(&mut writer)?;
...
fn choose_transfer_encoding(
    status_code: StatusCode,
    request_headers_opt: Option<&Headers>,
    http_version: &HTTPVersion,
    ...
    let user_request = if let Some(request_headers) = request_headers_opt {
        request_headers
            // finding TE
            .get("TE")
            // getting its value
    ...
    R: Read,
{
    ...
    pub fn new(
        ...
        let mut response = Response {
            reader: data,
            status_code,
            headers: Headers::with_capacity(16),
            data_length,
        ...
        H: Into<Header>,
    {
        ...
        // if the header is Content-Type and it's already set, overwrite it
            ...
            if let Some(content_type_header) = self.headers.get_header_mut("Content-Type") {
    ...
    ///  special behavior. See the documentation above.
    #[inline]
    pub fn with_headers<H>(mut self, headers: H) -> Result<Response<R>, HeaderError>
    where
        H: TryInto<Headers, Error = HeaderError>,
    {
        let headers = headers.try_into()?;

        headers.0.into_iter().for_each(|v| self.add_header(v));
    ...
    pub fn raw_print<W: Write>(
        ...
        if self.headers.contains("Date") == false {
            self.headers.insert_header(0, build_date_header());
        }

        // add `Server` if not in the headers
        if self.headers.contains("Server") == false {
            self.headers.insert_header(0, Header::from(&HEADER_SERVER));
        }

        // handling upgrade
        if let Some(upgrade) = upgrade {
            self.headers.insert_header(0, Header::from_str("Upgrade", upgrade).unwrap());
            self.headers.insert_header(0, Header::from(&HEADER_CONNECTION));
        ...
        let (mut reader, data_length): (Box<dyn Read>, _) = match (self.data_length, transfer_encoding) {
        ...
        let do_not_send_body = do_not_send_body == true
            || (self.status_code.in_range_inc(StatusCode::CONTINUE, StatusCode(199)) == true
                || self.status_code == StatusCode::NO_CONTENT
                || self.status_code == StatusCode::NOT_MODIFIED);

        // preparing headers for transfer
        match transfer_encoding {
            Some(TransferEncoding::Chunked) => self.headers.push(Header::from(&HEADER_TRANSFER_ENCODING)),
...
impl Response<File> {
    ...
    pub fn from_file(file: File) -> Response<File> {
        ...
        Response::new(StatusCode(200), Headers::new(), file, file_size, None)
    }
}
...
impl Response<Cursor<Vec<u8>>> {
        ...
        D: Into<Vec<u8>>,
    {
        ...
        Response::new(StatusCode::OK, Headers::new(), Cursor::new(data), Some(data_len), None)
    }
        ...
        S: Into<String>,
    {
        ...
        Response::new(
            StatusCode::OK,
            vec![Header::from(&HEADER_CONTENT_TYPE)].into(),
...
impl Response<io::Empty> {
    ...
    where
        S: Into<StatusCode>,
    {
        Response::new(status_code.into(), Headers::new(), io::empty(), Some(0), None)
    }
