use crate::{HTTPVersion, Header, Headers, Method, Request, request::new_request};
...
impl From<TestRequest> for Request {
    fn from(mut mock: TestRequest) -> Request {
        ...
        if false == mock.headers.get("Content-Length").is_some() {
            mock.headers.push(Header::from_str("Content-Length", mock.body.len().to_string()).unwrap());
        }
        new_request(
            mock.secure,
            mock.method,
