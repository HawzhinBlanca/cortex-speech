...
impl NativeTlsStream {
    pub(crate) fn peer_addr(&mut self) -> std::io::Result<Option<SocketAddr>> {
        self.0.lock().expect("Failed to lock SSL stream mutex").get_mut().peer_addr()
    }

    pub(crate) fn shutdown(&mut self, how: Shutdown) -> std::io::Result<()> {
        self.0.lock().expect("Failed to lock SSL stream mutex").get_mut().shutdown(how)
    }
}
...
impl NativeTlsContext {
    ...
    pub fn accept(&self, stream: Connection) -> Result<NativeTlsStream, Box<dyn Error + Send + Sync + 'static>> {
