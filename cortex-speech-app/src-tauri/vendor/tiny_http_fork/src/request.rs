...
use crate::util::{DeadlineReader, EqualReader, FusedReader, REQUEST_READ_DEADLINE};
use crate::{HTTPVersion, Headers, Method, Response, StatusCode};
...
where
    ...
    W: Write + Send + 'static,
{
    // Give every request body an absolute deadline. The socket timeout bounds a
    // completely idle peer; this wrapper also bounds a peer that trickles bytes.
    let mut source_data = DeadlineReader::new(source_data, REQUEST_READ_DEADLINE);

    // finding the transfer-encoding header
    let transfer_encoding_opt = headers.get("Transfer-Encoding");

    let content_length: Option<usize> = headers.get("Content-Length").and_then(|h| FromStr::from_str(h.as_str()).ok());
    ...
    let expects_continue = {
        let res = headers.get("Expect").map(|h| h.as_str());
    ...
    let connection_upgrade = {
        match headers.get("Connection").map(|h| h.as_str()) {
    ...
    let reader = if connection_upgrade == true {
        ...
        let params = transfer_encoding.as_str().split(',').map(|v| v.trim_start()).collect::<Vec<&str>>();
...
impl Request {
    ...
    pub fn upgrade<R: Read>(mut self, protocol: &str, response: Response<R>) -> io::Result<Box<dyn ReadWrite + Send>> {
        ...
        self.response_writer.as_mut().unwrap().flush()?; // TODO: unused result
        ...
        if let Some(sender) = self.notify_when_responded.take() {
            ...
            return Ok(Box::new(stream) as Box<dyn ReadWrite + Send>);
        } else {
            return Ok(Box::new(stream) as Box<dyn ReadWrite + Send>);
        }
    }
