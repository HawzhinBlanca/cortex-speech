...
- fixing docs.rs, missing ffff flags.
- warning cleanup


## 0.12.13

- common.rs added more From/TryFrom implementations for Headers and Header
- response.rs added with_headers()
- response.rs updated Response::new()
- edited documentation link.

## 0.12.12

// --- SCRIPT ERROR, MISSING DATA --- //

## 0.12.11

- added a messages_queue_channel which is based on the crossbeam mpsc.
- updated error handling for queue_channel
- added info() in log.rs
- added const headers in common.rs
- error handling is modified. Now incoming_requests() returns the request as a result which in case of
error reports the error.
- fixed UTF8 in headers support, the parser used AsciiString for parsing while in UTF8 mode.
