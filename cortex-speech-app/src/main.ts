...
if (import.meta.env.DEV && !("__TAURI_INTERNALS__" in window)) {
  const listKinds = /(^get_segments$|^list_|_reports$|_events$|_runs$|_history$|^search_|_queue$|^get_speakers$)/;
  const objKinds = /(^get_settings$|^app_health$|^db_info$|^import_status$|readiness$|^models_status$|_info$|^get_stats$|^compute_stats$)/;
