<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      notifications.error($t('diagnostics.loadFailed'), { cause: e });
    } finally {
      loading = false;
    }
  }
  ...
  async function clear() {
    ...
    try {
      ...
      notifications.error($t('diagnostics.clearFailed'), { cause: e });
    }
  }
...
<section class="space-y-3" data-testid="diagnostics-panel">
  <div class="flex items-center justify-between">
    <h3 class="text-sm font-medium text-cortex-100">{$t('diagnostics.title')}</h3>
  ...
  {#if !tauriAvailable}
    <p class="text-xs text-muted">{$t('diagnostics.desktopRequired')}</p>
  {:else if loading}
    <p class="text-xs text-muted">{$t('diagnostics.loading')}</p>
  {:else}
    {#if stats}
      <div class="grid grid-cols-2 gap-2" data-testid="diagnostics-stats">
        <div class="rounded-md border border-cortex-700/40 bg-cortex-900/30 p-2">
          ...
          <div class="text-[10px] text-muted">{$t('diagnostics.spansRecorded')}</div>
        </div>
        <div class="rounded-md border border-cortex-700/40 bg-cortex-900/30 p-2">
          <div
          ...
          <div class="text-[10px] text-muted">{$t('diagnostics.failures')}</div>
        </div>
        <div class="rounded-md border border-cortex-700/40 bg-cortex-900/30 p-2">
          <div class="text-lg font-semibold text-cortex-100">{ms(stats.avg_duration_ms)}</div>
          <div class="text-[10px] text-muted">{$t('diagnostics.averageDuration')}</div>
        </div>
        <div class="rounded-md border border-cortex-700/40 bg-cortex-900/30 p-2">
          <div class="text-lg font-semibold text-cortex-100">{ms(stats.total_duration_ms)}</div>
          <div class="text-[10px] text-muted">{$t('diagnostics.totalDuration')}</div>
        </div>
      </div>
    {/if}
        ...
        {#each spans as s, i (i)}
          ...
          >
            <span class="flex items-center gap-2">
              <span class={s.success ? 'text-emerald-300' : 'text-amber-300'}>
                {s.success ? $t('diagnostics.succeeded') : $t('diagnostics.failed')}
              </span>
