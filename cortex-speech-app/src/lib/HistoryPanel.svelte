...
<div class="flex flex-col h-full bg-slate-950/40 backdrop-blur-md rounded-xl border border-slate-800/40 p-4 space-y-4">
    ...
    <div class="flex gap-1.5">
      <button
        class="px-3 py-1.5 rounded-lg border text-xs font-medium transition-all duration-150 flex items-center gap-1 relative
          {canUndo ? 'bg-indigo-600/20 text-indigo-300 border-indigo-500/30 hover:bg-indigo-600/30' : 'bg-transparent text-slate-600 border-slate-900 cursor-not-allowed opacity-40'}"
        onclick={handleUndo}
        disabled={!canUndo}
        title="Undo (Ctrl+Z)"
      >
        ...
        {#if showHotkeyOverlay && canUndo}
          <span class="absolute -top-1.5 -right-1.5 bg-cyan-400 text-black text-[8px] font-mono font-bold px-1 rounded shadow-md border border-cyan-500 select-none z-50 pointer-events-none">^Z</span>
        {/if}
      </button>
      
      <button
        class="px-3 py-1.5 rounded-lg border text-xs font-medium transition-all duration-150 flex items-center gap-1 relative
          {canRedo ? 'bg-indigo-600/20 text-indigo-300 border-indigo-500/30 hover:bg-indigo-600/30' : 'bg-transparent text-slate-600 border-slate-900 cursor-not-allowed opacity-40'}"
        onclick={handleRedo}
        disabled={!canRedo}
    ...
    {:else}
      <div class="space-y-1.5">
        {#each localEdits as log}
          <div class="flex items-start gap-2.5 p-2 rounded-lg bg-slate-900/30 border border-slate-900/60 hover:bg-slate-900/50 transition-colors">
            <!-- Mini icon for type -->
            <span class="mt-0.5 w-1.5 h-1.5 rounded-full shrink-0
              {log.type === 'verify' ? 'bg-emerald-500' : log.type === 'delete' ? 'bg-red-500' : log.type === 'import' ? 'bg-blue-500' : 'bg-indigo-500'}">
            </span>
            <div class="flex-1 min-w-0">
              <p class="text-[11px] font-mono text-slate-300 leading-normal break-words">{log.description}</p>
