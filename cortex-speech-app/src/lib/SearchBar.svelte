...
<div class="space-y-2" data-testid="search-bar">
  <div class="relative">
    <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-cortex-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {#if query}
      {#if $searchLoading}
        <svg class="absolute right-8 top-1/2 -translate-y-1/2 w-4 h-4 text-cortex-400 animate-spin" fill="none" viewBox="0 0 24 24" aria-hidden="true">
  ...
  <div class="flex items-center gap-2">
    <div class="flex gap-1">
      {#each [{ label: $t('all'), value: null }, { label: $t('filterVerified'), value: true }, { label: $t('filterPending'), value: false }] as opt}
        <button
          class="text-xs px-2 py-1 rounded transition-colors {$filterVerified === opt.value ? 'bg-cortex-700 text-cortex-100' : 'text-cortex-400 hover:text-cortex-200'}"
