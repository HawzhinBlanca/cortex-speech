...
>
> **Not yet built (follow-up):** batch-import-path firing (needs a where-does-fired-text-live decision so it doesn't clobber `raw_transcript`); fairseq2 base-card resolution + `~/.config` write + LoRA-merge (need WSL); FE registry/firing-toggle panels (FE lane); threshold calibration + over-trigger gating (need the gold set); LOOP 1/2/3 (GER, KenLM, periodic LoRA).
