...
> **Remaining gap (roadmap M2b) — RESOLVED 2026-06-23.** `settings.language = "ckb"` **IS** passed into the sherpa OmniASR stream (`asr.rs:425-427`, `stream.set_option("language", &self.language)`) — the earlier "not passed" claim is stale. **A/B SPIKE FINDING:** on a real 10s Kurdish clip, the transcript is **IDENTICAL** with `hint="ckb"` vs no hint (both `"jóg dú mali"`), so **OmniASR-CTC ignores the `language` option — the hint is a confirmed NO-OP.** Worse, the generalist model emitted **Latin-script, non-Kurdish** output, confirming it needs effective language conditioning (or fine-tuning) before a real ckb scorecard is meaningful. Reproduce: `CORTEX_CKB_AB_CLIP=<wav> cargo test --manifest-path src-tauri/Cargo.toml --test real_audio ckb_language_hint_ab -- --ignored --nocapture`. See `docs/BLUEPRINT_9_5.md` M1.2.
