...
**Current canon (2026-08-23):** one exact 6,922-clip Lamo focus. The first-pass reviewer completes
the first pass; only then does the second-pass reviewer receive a blind, independent second pass.
Both identities live in `<data_dir>/reviewer_dialects.json`, never in this repository. Payment is
...
- Never let the two reviewers work the same campaign phase concurrently.
- Never activate the second-pass reviewer until the atomic transition proves all 6,922 effective
  first-pass phone decisions.
...
5. Atomically prove first-pass completion, freeze the focus in SQLite, and activate the second-pass
   reviewer:
...
Success must report `second_pass_active`, `authorizedReviewer` equal to the rostered second-pass
reviewer, `registeredFocus.segmentCount: 6922`, `independentPending: 6922`, zero adjudications,
zero conflicts, clean SQLite checks, and zero foreign-key violations. Any mismatch is a hard stop;
there is no force option.

6. Start the tested Cortex release, run the live supervision/link/queue probes, then release only
   the second-pass reviewer's already-issued link. The first-pass reviewer remains paused.

## Second-pass completion → adjudication
