# Progress Ledger — Archive: 2026-07-02 → 2026-07-24

Historical entries split out of `PROGRESS_LEDGER.md` (2026-07-26) to keep the live ledger
readable. **Nothing here was edited** — these are the original entries verbatim, in order.
The live ledger keeps the governance sections and the current era (iteration 167 onward).
...
already had <this-pc> + iPhone 15 Pro Max; verified via tailscale status). feat(couch) <hash>:
...
- Panel showed BOTH URLs as designed: Wi-Fi http://<lan-ip>:8737/?t=... AND Tailscale
  http://<this-pc-tailnet-ip>:8737/?t=... (matches the owner's tailnet <this-pc> exactly).
