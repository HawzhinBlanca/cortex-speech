...
**Gate-status authority: [STATUS.md](STATUS.md)** — this tracked page is intentionally static. The
actual generated status is hash-bound inside the completed proof bundle published by `verify_10.py`.
Prose in this file describes what each leg *means* and what is owner-gated; it deliberately
does not restate which legs currently pass, because that is exactly the claim that rots (this document
asserted `egress-runtime` was `not-built` for weeks after it shipped). Do **not** describe the app as
"10/10" until `verify_10.py` prints all-green on a clean checkout AND the owner-gated checklist below
is genuinely done.
