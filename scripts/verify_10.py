...
GATES = [
    ...
    ("review-pilot-certification", 2, "cmd", f'"{sys.executable}" "{APP / "scripts" / "check_review_pilot_certification.py"}" --exe "{EXE}"', APP, None, "Final paid-canary truth, distinct from capacity readiness: the exact Hawzhin and Pavel schema-60 policy must have 10+10 distinct corpus decisions and 2+2 durable hidden decisions, zero skips, hidden 2/2 at noticed=1/CER0, exact playback identity and >=85% coverage for ALL 24 corpus+hidden actions, and 24 matching compensation ledger/operation receipts. An incomplete or empty canary is RED, never a healthy-pool false pass."),
    ...
    ("playback-enforcement-readiness", 2, "cmd", f'"{sys.executable}" "{APP / "scripts" / "check_playback_enforcement_readiness.py"}" --exe "{EXE}"', APP, None, "Paid-review corpus listening proof for the EXACT deployed binary: the build contains the refusal guard, at least 20 post-build phone decisions cover at least two reviewer browsers, and every landed corpus decision carries >=85% playback evidence bound to reviewer, revision, and the canonical decoded-PCM BLAKE3 content hash. The final pilot certificate separately covers the four hidden actions. No env probe and no --since override: a missing binary/database or empty current-build window is RED, never skipped or backdated into a pass."),
