...
GATES = [
    ...
    ("review-mode-certification", 2, "cmd", f'"{sys.executable}" "{APP / "scripts" / "check_review_pilot_certification.py"}"', APP, None, "Mode-selected final review authority. An active flexible pool (introduced at schema 63; current authority schema 65) must bind to the exact hash-verified immutable release and pass a fresh detached full-integrity certification for its exact registry, membership, audio, rights, disk, local/offsite snapshots and internally consistent resolution totals; review readiness does not impersonate final dataset completion. Without a flexible pool, the strict legacy two-person canary remains mandatory: 10+10 corpus and 2+2 hidden decisions, zero skips, exact playback and complete ledger/operation receipts. Conflicting modes or missing evidence are RED."),
    ...
    try:
        with acquired_lease(lease) as abandoned_token:
            ...
            if abandoned_token is not None:
                ...
                failure_record = journal.append("phase_end", exitCode=2, verdict="INCOMPLETE_STALE_TAKEOVER")
                phase_ended = True
            ...
            if timed_out or return_code != 0:
                ...
                print(f"RUST COVERAGE PREREQUISITE FAILED @ {failure_record['at']}: exit={return_code}")
                return 1
        ...
        with contextlib.suppress(EvidenceError, OSError):
            if not phase_ended:
                ...
                failure_record = journal.append("publication_failure", exitCode=130, verdict="ABORTED")
        ...
        with contextlib.suppress(EvidenceError, OSError):
            if not phase_ended:
                failure_record = journal.append("phase_end", exitCode=1, verdict="VERIFIER_FAILURE", detail=str(error))
                phase_ended = True
            elif failure_record is None and not completion_published:
                failure_record = journal.append(
                    "publication_failure",
