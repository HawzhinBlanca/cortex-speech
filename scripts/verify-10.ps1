#!/usr/bin/env pwsh
# Cortex Speech - Windows entrypoint for the 10/10 verification gate.
# Mirrors `make verify-10` for environments without GNU make.
#
#   pwsh scripts/verify-10.ps1      # or:  powershell -File scripts/verify-10.ps1
#
# Exits non-zero (propagating verify_10.py's status) if any gate is red.
